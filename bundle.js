...
function double_free_reqs1(
  ...
  try {
    ...
    if (!success) {
      die('خطأ: فشل تحرير الذاكرة مرتين في منطقة تخصيص الذاكرة (malloc) بحجم 0x100');
    }
  }
}
//================================================================================================
...
async function get_patches(url) {
  ...
  if (!response.ok) {
    throw Error(`لم تكن استجابة الشبكة جيدة، الحالة: ${response.status}\n` + `فشل في جلب البيانات: ${url}`);
  }
...
async function patch_kernel(kbase, kmem, p_ucred, restore_info) {
  if (!is_ps4) {
    throw RangeError('ترقيع نواة نظام التشغيل PS5 غير مدعوم');
  }
  if ((config_target < 0x600) || (config_target >= 0x1000)) {
    throw RangeError('ترقيع النواة غير مدعوم');
  }
...
//  const buf = await get_patches(patch_elf_loc);
  ...
  switch (Console_FW_Version) {
    ...
    case "9.60":
      ...
      break;
    default:
      die('لم يتم العثور على ملف kpatch_bin');
      break;
  }
...
function runBinLoader() {
  ...
  log('برنامج BinLoader جاهز. أرسل حمولة البيانات إلى المنفذ 9020 الآن');
}
//================================================================================================
// Malloc ========================================================================================
//================================================================================================
...
function Init_Globals() {
  ...
  if (mem === null) {
    window.log("خطأ: الذاكرة غير مهيأة. ربما فشلت عملية الاستغلال.\nيرجى تحديث الصفحة والمحاولة مرة أخرى...", "red");
    return 0;
  }
  // Kernel offsets
  switch (Console_FW_Version) {
    ...
    case "9.60":
      ...
      break;
    default:
      throw "البرامج الثابتة غير المدعومة";
  }
  // ROP offsets
  switch (Console_FW_Version) {
    case "7.00":
...
function checkPlatformIsSupported() {
  ...
  src_part = res;
  window.log("تم الكشف عن: PS4" + device + " v" + fwVersion + ", إصدار الثغرة الأمنية: v2.04\n");
  // Supported FW lists
  var supportedFW = {
...
async function doJBwithPSFreeLapseExploit() {
  if (!checkPlatformIsSupported()) {
    window.log("السوفت الخاص بك لاغير مدعومة! مصمم لأجهزة PS4 سوفت وير من 7.00 إلى 9.60", "red");
    /*
  ...
  window.log("بدء تشغيل ثغره الاستغلال.....");
  try {
    window.log("PSFree STAGE 1/3: UAF SSV");
    ...
    window.log("حدث خطأ أثناء تشغيل الثغره\nيرجى تحديث الصفحة والمحاولة مرة أخرى...\nتعريف الخطأ: " + error, "red");
    return;
  }
  window.log("بدء تشغيل ثغرة استغلال نواة النظام...");
  ...
  try {
    ...
    if (jb_step_status !== 1) {
      window.log("لم يتم تهيئة المتغيرات العامة بشكل صحيح. يرجى إعادة تشغيل وحدة التحكم والمحاولة مرة أخرى...", "red");
      return;
    }
    await lapse_init();
    ...
    try {
      if (sysi('setuid', 0) == 0) {
        window.log("\nنجحت عملية استغلال ثغرة", "green");
        await sleep(500); // Wait 500ms
        // Inject HEN payload
        jb_step_status = await PayloadLoader("payload.bin"); // Read payload from .bin file
        if (jb_step_status !== 1) {
          window.log("فشل تحميل HEN!\nيرجى إعادة تشغيل الجهاز والمحاولة مرة أخرى...", "red");
          return;
        }
        window.log("تحميل GOLDHEN", "green");
        window.log("\nنجحت عملية استغلال ثغرة");
      } else {
        window.log("حدث خطأ أثناء فترة التقادم\nيرجى إعادة تشغيل الجهاز والمحاولة مرة أخرى...", "red");
        return;
      }
    } catch {
      // لم يتم استغلالها بعد، لقد حدث خطأ ما. لكنها وصلت إلى هنا...
      die("فشلت عملية استغلال نواة النظام!");
    }
  } catch (error) {
    window.log("حدث خطأ أثناء فترة التقادم\nيرجى إعادة تشغيل الجهاز والمحاولة مرة أخرى...\nتعريف الخطأ: " + error, "red");
  }
}
